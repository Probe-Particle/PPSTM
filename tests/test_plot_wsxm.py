...
_DX = (_SCAN_WINDOW[1,0]-_SCAN_WINDOW[0,0])/_SCAN_DIM[0]
_DY = (_SCAN_WINDOW[1,1]-_SCAN_WINDOW[0,1])/_SCAN_DIM[1]
_DZ = (_SCAN_WINDOW[1,2]-_SCAN_WINDOW[0,2])/_SCAN_DIM[2]
_TIP_R0 = np.mgrid[
    _SCAN_WINDOW[0,0]:_SCAN_WINDOW[1,0]+0.0001:_DX,
    _SCAN_WINDOW[0,1]:_SCAN_WINDOW[1,1]+0.0001:_DY,
    _SCAN_WINDOW[0,2]:_SCAN_WINDOW[1,2]+0.0001:_DZ
].transpose()
...
def test_didv_states_scan_filename_matches_convention(tmp_path: Path, scan_type):
    """Verify WSxM filenames for dIdV and states scans follow the required format."""

    expected_file_name = _build_expected_didv_file_name(work_function_decay=0.)

    _plot_wsxm(scan_type,
              didv=_DIDV,
              save_dir=tmp_path)
...
def test_stm_scan_filename_matches_convention(tmp_path: Path, scan_type):
    """Verify WSxM filenames for STM scans follow the required format."""
...
def test_voltage_scan_filename_matches_convention(tmp_path: Path, scan_type):
    """Verify WSxM filenames for voltage scans follow the required format."""
...
def _build_expected_didv_file_name(work_function_decay: float = _WORK_FUNCTION_DECAY) -> str:
    return f'didv_{_NAMES[0]}_tip_{_TIP_TYPE}-{_TIP_ORB}' \
           f'_WF_{_WORK_FUNCTION - _VOLTAGES[0] * work_function_decay}' \
           f'_eta_{_ETA:.1f}_{0:03d}.{_FILENAME_EXTENSION}'
...
def _plot_wsxm(scan_type: str, save_dir: Path, current: Optional = None, didv: Optional = None):
    visualization.plot_wsxm(_config(scan_type),
                            current=current,
                            didv=didv,
                            voltages=_VOLTAGES,
                            names=_NAMES,
