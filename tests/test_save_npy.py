...
def test_didv_states_scan_filename_matches_convention(tmp_path: Path, scan_type):
    """Verify NPY filenames for dIdV and states scans follow the required format."""
...
def test_stm_scan_filename_matches_convention(tmp_path: Path, scan_type):
    """Verify NPY filenames for STM scans follow the required format."""
...
def test_voltage_scan_filename_matches_convention(tmp_path: Path, scan_type):
    """Verify NPY filenames for voltage scans follow the required format."""
...
def _save_npy(scan_type: str, save_dir: Path, current: Optional = None, didv: Optional = None):
    visualization.save_npy(_config(scan_type),
                           current=current,
                           didv=didv,
                           voltages=_VOLTAGES,
                           names=_NAMES,
                           lvec=_LVEC,
                           atomic_info_or_head=_ATOMIC_INFO_OR_HEAD,
                           save_dir=save_dir)
