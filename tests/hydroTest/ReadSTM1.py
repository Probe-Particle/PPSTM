...
def pbc_kpoints(coeffsRe,coeffsIm,kpoints,Ratin,eigenvec):
	'''
	Applying PBC to the LCAO Coefficients
	'''
	ns=len(eigenvec)
	eigenvec_new=np.zeros(ns*4)
	for i in range(4):
		eigenvec_new[i*ns:ns*(i+1)]=eigenvec[:]
	# change global number of states here
	if ((pbc_ != (0,0))or(pbc_ != (0.0,0.0))) :
		print "applying pbc"
		coeffRe =np.repeat(coeffsRe,int(pbc_[0]*2+1)*int(pbc_[1]*2+1),0)#.flatten()
		coeffIm =np.repeat(coeffIm,int(pbc_[0]*2+1)*int(pbc_[1]*2+1),0)#.flatten()
		global num_at_;	num_at_ *=int(pbc_[0]*2+1)*int(pbc_[1]*2+1)
		coeff=np.zeros((4*(n_max_-n_min_),num_at_,Ynum_))
		kpn=0;
		i=0 # Real*Cos
		for ii in range((n_max_-n_min_)):
			for iat range(num_at_):   #   cos (k.Ra)                                                                           * kpoint weight * Re coefs         
...
def     handle_coef_w_k(coefRe,coefIm,kpoints,Ratin,eigenvec)
	...
	#return pbc_coef(coeffs);
	return pbc_kpoints(coeffsRe,coeffsIm,kpoints,Ratin,eigenvec)
...
#	elif (len(kaka) > 2 and type(kpoint) == np.ndarray and len(kpoint) == 4):  #type(kpoint) == list()
		...
		# lowering over atoms and applying PBC
		coeffs,eigenvec = handle_coef_w_k(coefRe,coefIm,kpoint,Ratin,eigenvec)
		gMode = 0
		print "All coefficients read, band mode ON"
	else:
		print "There's an issue in the k-points, bu bu"

	return eig.copy(), coeffsRe.copy(), coeffsIm.copy(), Ratin.copy(), gMode.copy();
