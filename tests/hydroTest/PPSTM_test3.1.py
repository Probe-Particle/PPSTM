...
kInputName = 'phik_0001_'   # This is for fireball input files only. Select your k-point file. No need to put in the orbital names.
...
geometry_file = 'start.xyz' # E.G. 'input.xyz' , 'input.bas' , 'geometry.in'; None for GPAW #
pbc           = (2,0)        # (0,0) = None = False -- only original geometry ; (0.5,0.5) -- 2x2 cell ; (1,1) -- 3x3 cell (around original) ; (2,2) -- 5x5 cell (around original) ... #
lvs           =  'start.lvs'  #[[6.89188341,0.0],[0.0,30]]        # None ; [[ax,ay,0],[bx,by,0]],[0,0,cz]] or [[ax,ay],[bx,by]] ; 'input.lvs' -- files with specified cell ; in FHI-AIMS & GPAW allready specified with geometry #
...
#
x = [   0.0, 34.0, 0.1 ]     # [xmin, xmax, dx] #
y = [ -10.0, 10.0, 0.1 ]     # [ymin, ymax, dy] #
z = [  5.91, 5.91, 0.01  ]     # !!!! z-starts from zero - normally zmin >= 3-4 Ang above heighest atoms !!!! [zmin, zmax, dz] ; for single height scan use : [z, z, 1.0] #
#
...
files_path   = '/storage/praha4-fzu/home/shayaned/fireball/bispentacene/'            # where are files fron DFT code ; rather do not use this #
#
#
##########################################################################################################################
#                                                                                                                        #
...
if (plot_atoms):
    geom_plot, tmp1, tmp2 = Bu.loadAtoms('start.xyz'); del tmp1, tmp2;
...
if ((dft_code == 'fireball') or(dft_code == 'Fireball') or (dft_code == 'FIREBALL')):
    if (mode == 'real'):
       eigEn, coefs, Ratin = RS.read_FIREBALL_real(name = files_path + kInputName, geom=files_path+geometry_file, lvs = cell, fermi=fermi, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max,cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs);
    elif (mode == 'imag'):
       eigEn, coefs, Ratin = RS.read_FIREBALL_imag(name = files_path + kInputName, geom=files_path+geometry_file, lvs = cell, fermi=fermi, orbs = sample_orbs, pbc=pbc, cut_min=cut_min, cut_max=cut_max,cut_at=cut_atoms, lower_atoms=lower_atoms, lower_coefs=lower_coefs);  
    elif (mode == 'all'):
...
#       print "ALL COEFS", coefs, coefs.shape
       ...
       print "Please pick a mode between real and imaginary!"    
...
#SHY
mat0 = genfromtxt('start.xyz', skip_header=2)
#data = pd.read_csv('input_plot.xyz', error_bad_lines=False)ss
data, target = np.array_split(np.loadtxt('start.xyz', skiprows=2, dtype=str), [-1], axis=1)
...
if ((scan_type == 'didv') or (scan_type == 'dIdV') or (scan_type == 'didv-single')):
   if ((mode == 'real') or (mode == 'imag')):
      didv    = np.array([   PS.dIdV( V,    WorkFunction, eta, eigEn, tip_r, Ratin, coefs, orbs=sample_orbs, s=tc[0], px =tc[1], py=tc[2], pz=tc[3], dz2=tc[4], dxz=tc[5], dyz=tc[6] ) ])
      didv_b = True; WF_decay= 0.0;
       #print "DEBUG: didv.shape ", didv.shape
   elif (mode == 'all'):
      didvR    = np.array([   PS.dIdV( V,    WorkFunction, eta, eigEn, tip_r, Ratin, coefs1, orbs=sample_orbs, s=tc[0], px =tc[1], py=tc[2], pz=tc[3], dz2=tc[4], dxz=tc[5], dyz=tc[6] ) ])
      didvI    = np.array([   PS.dIdV( V,    WorkFunction, eta, eigEn, tip_r, Ratin, coefs2, orbs=sample_orbs, s=tc[0], px =tc[1], py=tc[2], pz=tc[3], dz2=tc[4], dxz=tc[5], dyz=tc[6] ) ])
      didv     = didvR + didvI
      didv_b = True; WF_decay= 0.0;
   else:
...
#            print "namez[vv]:", namez[vv]
		...
		name_file = "./dIdV"+kInputName+'_'+str(Voltages)+tip_type+';'+tip_orb+';'+mode+namez[vv]+".dat"
		...
		plt.savefig( './dIdV'+kInputName+'_'+str(Voltages)+mode+"-"+namez[vv]+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'_%03d.png' %k , bbox_inches='tight' )
		plt.close()
	    if STM_b :
		# ploting part here:
		plt.figure( figsize=(0.5 * lvec[1,0] , 0.5 * lvec[2,1] ) )
