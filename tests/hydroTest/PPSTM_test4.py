...
#
ppstm_path = '/home/foxcatcher/Desktop/pavel/stm/PPSTM/'
#
# ***** Main informations ******
...
cut_min      = -5.0          # cut out all orbitals lower than  -2.5 eV below Fermi (should be: cut_min <= Vmin-2*eta) . taken to the Fermi Level #
cut_max      = +9.0          # cut out all orbitals higher than -2.5 eV above  Fermi (should be: cut_max >= Vmax+2*eta) . taken to the Fermi Level #
files_path   = '/home/foxcatcher/Desktop/pavel/stm/PPSTM/tests/hydroTest/4/'            # where are files fron DFT code ; rather do not use this #
#
#
##########################################################################################################################
#                                                                                                                        #
...
if (plot_atoms):
    geom_plot, tmp1, tmp2 = Bu.loadAtoms(files_path +'input.xyz'); del tmp1, tmp2;
...
#            print "namez[vv]:", namez[vv]
	    ...
	    name_plot=namez[vv]+';height: 5.0 $\AA$;tip:'+tip_type+';'+tip_orb+';'
	    if didv_b :
		# ploting part here:
		...
		name_file = "./test/TEST"+tip_type+';'+tip_orb+';'+namez[vv]+".dat"
		...
		plt.savefig( './test/TEST_Stip_5.0_'+'_'+"-"+namez[vv]+"_tip_"+tip_type+"-"+tip_orb+"_WF_"+str(WorkFunction-Voltages[vv]*WF_decay)+"_eta_"+str(eta)+'_%03d.png' %k , bbox_inches='tight' )
		plt.close()
	    if STM_b :
		# ploting part here:
		plt.figure( figsize=(0.5 * lvec[1,0] , 0.5 * lvec[2,1] ) )
