...
class TestDidvRaSign:
    ...
    _S_TIP_ORBITAL = np.asarray([1.0] + [0.0] * 8)
    ...
    def test_didv_r_a_sign(self, lcao_coefficients, didv_backend: Callable, rtol: float):
        """Check the correctness of the sign of r_a."""
    ...
    def _expected_didv(self, lcao_coefficients: np.ndarray):
        ...
        g = 16 * math.pi ** 3 * (self._s_tip_sp_sample_orbital_conductances(lcao_coefficients) * radial) ** 2
        return g.reshape(1, 1, 1)

    def _s_tip_sp_sample_orbital_conductances(self, lcao_coefficients: np.ndarray):
        """Conductances for different sp orbitals of sample on an s orbital tip."""
        r_a = (self._TIP_POSITIONS.squeeze() - self._SAMPLE_ATOMS_POSITIONS.squeeze()) * self._AB
