...
// ================= orbital conductances (for s tip only now)
 ...
 inline double ssp(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 ...
 inline double sspd(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 double f = coe[0];					                             // s  orb. of sample
 f += coe[1]*dR.y*rev_rr*N_p;		                             // py orb. of sample
 f += coe[2]*dR.z*rev_rr*N_p;		                             // pz orb. of sample
 f += coe[3]*dR.x*rev_rr*N_p;		                             // px orb. of sample
 f += coe[4]*dR.x*dR.y*sqr(rev_rr)*N_d*resD;                     //dxy orb. of sample
 f += coe[5]*dR.y*dR.z*sqr(rev_rr)*N_d*resD;                     //dyz orb. of sample
 f += coe[6]*( 3*sqr(dR.z)*sqr(rev_rr) - 1 )*N_d2*resD;          //dz2 orb. of sample
 f += coe[7]*dR.x*dR.z*sqr(rev_rr)*N_d*resD;                     //dxz orb. of sample
 f += coe[8]*0.5*sqr(rev_rr)*( sqr(dR.x) - sqr(dR.y) )*N_d*resD; //dx2-y2 orb. of sample
 return f;
 }

// ================= orbital conductances (for py tip only now)

 inline double pysp(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 ...
 inline double pyspd(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 double f = coe[0]*dR.y*decay;													// s  orb. of sample
 f += coe[1]*N_p*( -1 + decay*rev_rr*sqr(dR.y) + sqr(rev_rr)*sqr(dR.y) );		// py orb. of sample
 f += coe[2]*N_p*dR.y*dR.z*( decay*rev_rr + sqr(rev_rr) );						// pz orb. of sample
 f += coe[3]*N_p*dR.y*dR.x*( decay*rev_rr + sqr(rev_rr) );						// px orb. of sample
 f += coe[4]*N_d*dR.x*rev_rr*( 2*sqr(dR.y)*sqr(rev_rr) + decay*sqr(dR.y)*rev_rr - 1 )*resD;                           //dxy orb. of sample
 f += coe[5]*N_d*dR.z*rev_rr*( 2*sqr(dR.y)*sqr(rev_rr) + decay*sqr(dR.y)*rev_rr - 1 )*resD;                           //dyz orb. of sample
 f += coe[6]*N_d2*dR.y*( 6*sqr(dR.z)*trr(rev_rr) + decay*(3*sqr(dR.z)*sqr(rev_rr)-1) )*resD;                          //dz2 orb. of sample
 f += coe[7]*N_d*dR.x*dR.y*dR.z*sqr(rev_rr)*( 2*rev_rr + decay )*resD;                                                //dxz orb. of sample
 f += coe[8]*N_d*rev_rr*dR.y*( (sqr(dR.x)-sqr(dR.y))*sqr(rev_rr) + 0.5*decay*(sqr(dR.x)-sqr(dR.y))*rev_rr + 1 )*resD; //dx2-y2 orb. of sample
 return f*rev_rr;
 }

// ================= orbital conductances (for pz tip only now)

 inline double pzsp(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 ...
 inline double pzspd(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 double f = coe[0]*dR.z*decay;													// s  orb. of sample
 f += coe[1]*N_p*dR.z*dR.y*( decay*rev_rr + sqr(rev_rr) );						// py orb. of sample
 f += coe[2]*N_p*( -1 + decay*rev_rr*sqr(dR.z) + sqr(rev_rr)*sqr(dR.z) );		// pz orb. of sample
 f += coe[3]*N_p*dR.z*dR.x*( decay*rev_rr + sqr(rev_rr) );						// px orb. of sample
 f += coe[4]*N_d*dR.x*dR.y*dR.z*sqr(rev_rr)*( 2*rev_rr + decay )*resD   ;                               //dxy orb. of sample
 f += coe[5]*N_d*dR.y*rev_rr*( 2*sqr(dR.z)*sqr(rev_rr) + decay*sqr(dR.z)*rev_rr - 1 )*resD;             //dyz orb. of sample
 f += coe[6]*N_d2*( (6*trr(dR.z)*trr(rev_rr)-6*dR.z*rev_rr) + decay*(3*sqr(dR.z)*sqr(rev_rr)-1) )*resD; //dz2 orb. of sample
 f += coe[7]*N_d*dR.x*rev_rr*( 2*sqr(dR.z)*sqr(rev_rr) + decay*sqr(dR.z)*rev_rr - 1 )*resD;             //dxz orb. of sample
 f += coe[8]*N_d*sqr(rev_rr)*dR.z*(sqr(dR.x)-sqr(dR.y))*( rev_rr + 0.5*decay  )*resD;                   //dx2-y2 orb. of sample
 return f*rev_rr;
 }

// ================= orbital conductances (for px tip only now)

 inline double pxsp(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 ...
 inline double pxspd(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
 double f = coe[0]*dR.x*decay;													// s  orb. of sample
 f += coe[1]*N_p*dR.x*dR.y*( decay*rev_rr + sqr(rev_rr) );						// py orb. of sample
 f += coe[2]*N_p*dR.x*dR.z*( decay*rev_rr + sqr(rev_rr) );						// pz orb. of sample
 f += coe[3]*N_p*( -1 + decay*rev_rr*sqr(dR.x) + sqr(rev_rr)*sqr(dR.x) );		// px orb. of sample
 f += coe[4]*N_d*dR.y*rev_rr*( 2*sqr(dR.x)*sqr(rev_rr) + decay*sqr(dR.x)*rev_rr - 1 )*resD;                           //dxy orb. of sample
 f += coe[5]*N_d*dR.x*dR.y*dR.z*sqr(rev_rr)*( 2*rev_rr + decay )*resD;                                                //dyz orb. of sample
 f += coe[6]*N_d2*dR.x*( 6*sqr(dR.z)*trr(rev_rr) + decay*(3*sqr(dR.z)*sqr(rev_rr)-1) )*resD;                          //dz2 orb. of sample
 f += coe[7]*N_d*dR.z*rev_rr*( 2*sqr(dR.x)*sqr(rev_rr) + decay*sqr(dR.x)*rev_rr - 1 )*resD;                           //dxz orb. of sample
 f += coe[8]*N_d*rev_rr*dR.x*( (sqr(dR.x)-sqr(dR.y))*sqr(rev_rr) + 0.5*decay*(sqr(dR.x)-sqr(dR.y))*rev_rr - 1 )*resD; //dx2-y2 orb. of sample
 return f*rev_rr;
 }

// ================= orbital conductances (for dxz tip only now)

 inline double dxzsp(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
...
// ================= orbital conductances (for dyz tip only now)
 ...
 inline double dyzsp(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
...
// ================= orbital conductances (for dyz tip only now)
 ...
 inline double dz2sp(double* coe, double rev_rr, const Vec3d& dR, double resD){
 //printf("inside ssp \n");
