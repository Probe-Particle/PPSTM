...
class ProbeStmNumpy(ABC):
    ...
    def __init__(self, V, WF, eta, eig: np.ndarray, R: np.ndarray, Rat: np.ndarray, coes: np.ndarray, tip_coes):
        ...
        self._eta = eta
        self._eig = eig
        self._sample_atom_position = Rat                       # shape                (n_a, 3)
        self._tip_position = np.expand_dims(R, 3)         # shape (n_x, n_y, n_z,   1, 3)
        self._tip_coes = tip_coes
        self._coes = coes.reshape(len(eig),
                                  len(Rat),
    ...
    def __call__(self) -> np.ndarray:
        ...
        r_a = np.expand_dims(
            (self._sample_atom_position - self._tip_position) * self._AB,
            axis=3,
        )                                               # shape (n_x, n_y, n_z, 1, n_a, 3)
        r_a_norm = np.linalg.norm(r_a, ord=2, axis=-1)  # shape (n_x, n_y, n_z, 1, n_a)
        ...
        for i, fn in enumerate(self.orbital_conductances_fns):
            if self._tip_coes[i] > 0.:
                orbital_conductances_i = fn(r_a, r_a_norm, decay)                                                       # shape (n_x, n_y, n_z,   1, n_a, orb_t)
                orbital_conductances_i = np.einsum("...i,...i", orbital_conductances_i, self._coes)  # shape (n_x, n_y, n_z, n_o, n_a)

                orbital_conductances.append(orbital_conductances_i)
                tip_coes_shrink.append(self._tip_coes[i])

        orbital_conductances = np.stack(orbital_conductances, axis=3)                                  # shape (n_x, n_y, n_z, n_t, n_o, n_a)

        radial = np.expand_dims(np.exp(-decay * r_a_norm), axis=3)                                     # shape (n_x, n_y, n_z,   1,   1, n_a)
        g_atom = np.einsum("...a,...a", orbital_conductances, radial)               # shape (n_x, n_y, n_z, n_t, n_o)

        sample_dos = self._eta / ((2 * np.pi) * ((self._eig - self._v) ** 2 + 0.25 * self._eta ** 2))  # shape                     (n_o,), Lorentzian
        g = np.dot(g_atom**2, sample_dos)                                                              # shape (n_x, n_y, n_z, n_t)

        tip_coes_shrink = np.asarray(tip_coes_shrink)                                                  # shape                (n_t,)
        g = np.dot(g, tip_coes_shrink)                                                                 # shape (n_x, n_y, n_z)

        g = g * 16 * np.pi**3 * decay
    ...
    def _py(self, r_a: np.ndarray, r_a_norm: np.ndarray, decay: float) -> np.ndarray:
        ...
        return np.stack(t, axis=-1) * np.expand_dims(r_a_norm_inv, axis=-1)
...
class ProbeStmNumpySp(ProbeStmNumpy):
    ...
    def _dxz(self, r_a: np.ndarray, r_a_norm: np.ndarray, decay: float) -> np.ndarray:
        ...
        return np.stack(t, axis=-1) * np.expand_dims(r_a_norm_inv_sq, axis=-1)
